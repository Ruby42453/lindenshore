-- 1. Sandwich-attack heuristic
-- Flags blocks where the same address opens and closes the block's trades
-- in opposite directions around one or more other traders' swaps.
-- Heuristic only: misses router-mediated sandwiches (same sender on every
-- leg) and can occasionally flag unrelated repeat trades.
WITH ordered AS (
    SELECT
...
-- 2. Price impact by trade-size bucket
-- Tick delta between consecutive swaps as a proxy for price impact.
-- Buckets on trade_value_quote_units (USD-normalized) rather than raw
-- amount0, since token0 differs in decimals and asset per chain.
WITH deltas AS (
    SELECT
        chain,
...
-- 3. Hourly volume and swap count
-- Uses block_timestamp (interpolated) rather than block_number so this is
-- comparable across chains with different block times.
SELECT
    chain,
...
-- Buckets both chains into 1-minute windows and computes the % spread.
-- Uses price_quote_per_base, not price_token1_per_token0, since token0/
-- token1 order differs between the two pools for the same nominal pair.
WITH bucketed AS (
    SELECT
        chain,
